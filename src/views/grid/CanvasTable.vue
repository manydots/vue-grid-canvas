    ...
    <div ref="grid" class="excel-table" :style="`height:${height + 2}px;`" @paste="doPaste">
        ...
        <transition name="slide-fade">
            <div v-if="showTip" class="tip">
                <i class="tip-icon" :style="'background-image:url(' + require(`${publicPath}/error.png`) + ');'"></i>
                {{ tipMessage }}
            </div>
        </transition>
        <!-- 表头设置 -->
        <transition name="slide-fade">
            <div v-if="showColumnSet" ref="columnSet" class="column-set">
                <div class="column-set__title">表头设置</div>
                <div class="column-set__content" :style="'max-height:' + (height - 105) + 'px;'">
                    <ul>
                        <li v-for="item of allColumns" :key="item.id" style="text-align: left">
                            <GeCheckbox v-model="item.checked">{{ item.title }}</GeCheckbox>
                        </li>
                    </ul>
                </div>
        ...
        <canvas ref="canvas" :width="width" :height="height" :style="`width:${width}px;height:${height}px;`" @contextmenu.prevent="handleContextMenu"></canvas>
    </div>
</template>

<script>
import mittMixin from '@/mixins/mittMixin';
...
export default {
    mixins: [mittMixin, calculate, painted, events, scroller, history],
    components: { GeCheckbox },
    props: {
        columns: Array,
        gridData: Array,
        type: {
    ...
    created() {
        ...
        this.$busOn('updateNoSave', (data) => {
            this.saveItems(data, false);
        });
        this.$busOn('update', (data) => {
            this.$emit('updateValue', this.saveItems(data, true));
        });
        this.$busOn('updateItem', (data) => {
    ...
    methods: {
        ...
        setValueTemp(e) {
            // console.log('setValueTemp', e.target);
            ...
            if (!this.isPaste) {
                ...
                if (startRowIndex - this.focusCell.rowIndex > 1 || lastCellIndex - startCellAt > 0) {
                    ...
                    this.selectArea = {
                        ...
                        cellIndex: this.focusCell.cellIndex,
                        rowCount: Math.abs(startRowIndex - this.focusCell.rowIndex),
                        offset: { ...this.offset }
                    };
                    this.isSelect = true;
                }
                this.$busEmit('update', modifyData);
            } else {
                this.isPaste = false;
