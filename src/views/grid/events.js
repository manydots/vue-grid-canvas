...
export default {
    ...
    methods: {
        removeEvent() {
            ...
            window.removeEventListener(this.isFirefox ? 'DOMMouseScroll' : 'mousewheel', this.handleWheel, { passive: false });
            window.removeEventListener('keydown', this.handleKeydown, false);
            window.removeEventListener('keyup', this.handleKeyup, false);
            // window.removeEventListener('handleContextMenu', this.handleContextMenu, false);
        },
        initEvent() {
            ...
            window.addEventListener(this.isFirefox ? 'DOMMouseScroll' : 'mousewheel', this.handleWheel, { passive: false });
            window.addEventListener('keydown', this.handleKeydown, false);
            window.addEventListener('keyup', this.handleKeyup, false);
            // window.addEventListener('handleContextMenu', this.handleContextMenu, false);
            ...
            if (!this.isSelect) {
                ...
                if (this.showCheckbox) {
                    if (x > this.serialWidth && x < this.originPoint.x) {
                        ...
                        console.log(`复选框点击区域X:${x},Y:${y}`, checkbox);
                        // console.log(checkbox);
                        let selectValue = checkbox?.id ?? checkbox?.rowIndex;
                        if (checkbox) {
                            // console.log(checkbox);
                            if (this.selected.indexOf(selectValue) !== -1) {
                                ...
                                this.selected.push(selectValue);
                            }
                            this.rePainted();
                            ...
                            if (this.selected.length === this.allRows.length) {
                                ...
                                for (const row of this.allRows) {
                                    let selectValue = row?.id ?? row?.rowIndex;
                                    this.selected.push(selectValue);
                                }
                            }
                            this.rePainted();
                        ...
                        this.$emit('multiSelect', this.selected);
                        return;
                    }
                }
        ...
        // handleMousedown 赋值focusCell数据
        handleDoubleClick() {
            console.log('focusCell', this.focusCell);
            if (this.focusCell) {
        ...
        handleContextMenu(evt) {
            if (evt.target.tagName === 'CANVAS') {
                ...
                // 根据需要进行相应操作或展示自定义菜单等
                console.log('触发了右键点击', eX, eY);
                console.log('contextMenu', cell);
            }
        },
        // 滑动
        handleWheel(e) {
                    ...
                    if (cell) {
                        if (cell.x >= x && cell.y >= y) {
                            this.selectArea = {
                                x,
                                y,
                                width: cell.x - x + cell.width,
                                height: cell.y - y + cell.height,
                                cellIndex,
                                rowIndex,
                                offset: { ...this.offset }
                            };
            ...
            if (evt.target.tagName === 'CANVAS') {
                setTimeout(() => {
                    ...
                    if (eX > this.originPoint.x && eY > this.rowHeight && eX < this.maxPoint.x) {
                        ...
                        this.rePainted();
                        // 点击表头/左侧序号
                        console.log('非表格内容区域 复选框逻辑在handleClick处理');
                    }
                }, 0);
                ...
                if (!this.isEditing) {
                    if (e.keyCode === 38) {
                        ...
                        if (this.isSelect) {
                            ...
                            this.$busEmit('update', deleteData);
                        } else {
                            this.$busEmit('updateItem', {
                        ...
                        if (e.keyCode === 90 && e.metaKey) {
                            e.preventDefault();
                            this.$busEmit('history_back');
                        } else if (e.keyCode === 89 && e.metaKey) {
                            e.preventDefault();
                            this.$busEmit('history_forward');
            ...
            if (type === 'up') {
                ...
                if (row !== this.displayCells[this.displayCells.length - 1][0].rowIndex) {
                    ...
                    if (rowIndex < this.allRows.length - 1) {
                        ...
                        const { displayCells } = this.rePainted();
                        this.focusCell = Object.assign({}, this.getDisplayCellByRowIndex(displayCells, displayCells[displayCells.length - 1][0].rowIndex, cell), {
                            offset: { ...this.offset }
                        });
                        this.paintFocusCell(this.focusCell);
                    }
                ...
                if (cell !== this.getFirstDisplayCellIndex(this.displayCells)) {
                    ...
                    if (cellIndex > 0) {
                        ...
                        const { displayCells } = this.rePainted();
                        this.focusCell = Object.assign({}, this.getDisplayCellByRowIndex(displayCells, row, this.getFirstDisplayCellIndex(displayCells)), {
                            offset: { ...this.offset }
                        });
                        this.paintFocusCell(this.focusCell);
                    }
                ...
                if (cell !== this.getLastDisplayCellIndex(this.displayCells)) {
                    ...
                    if (cellIndex < this.allColumns.length - 1) {
                        this.offset.x -= this.allColumns[cellIndex + 1].width;
                        const { displayCells } = this.rePainted();
                        this.focusCell = Object.assign({}, this.getDisplayCellByRowIndex(displayCells, row, this.getLastDisplayCellIndex(displayCells)), {
                            offset: { ...this.offset }
                        });
                        this.paintFocusCell(this.focusCell);
                    }
